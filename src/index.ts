...
export default class CanvasSelect extends EventBus {
  ...
  public mergeEvent(e: TouchEvent | MouseEvent) {
    ...
    if (this.isMobile && (e as TouchEvent).touches) {
      ...
      mouseX = Math.round(clientX - this.LEFT);
      mouseY = Math.round(clientY - this.TOP);
      if ((e as TouchEvent).touches?.length === 2) {
        const { clientX: clientX1 = 0, clientY: clientY1 = 0 } =
          (e as TouchEvent).touches[1] || {};
        mouseCX = Math.round(
          Math.abs((clientX1 - clientX) / 2 + clientX) - this.LEFT,
        );
        mouseCY = Math.round(
          Math.abs((clientY1 - clientY) / 2 + clientY) - this.TOP,
        );
      } else if ((e as TouchEvent).touches?.length === 1) {
        mouseCX = Math.round(clientX - this.LEFT);
        mouseCY = Math.round(clientY - this.TOP);
      }
    } else {
      ...
      if (this.ctrlIndex > -1 && !this.readonly) {
        ...
        if (this.activeShape.creating && !this.readonly) {
          ...
          if (this.isCtrlKey && this.activeShape.type) {
            ...
            hitShape =
              currentIdx >= 0 && currentIdx < hits.length - 1
                ? hits[currentIdx + 1]
                : hits[0];
  ...
  public async handleMouseMove(e: MouseEvent | TouchEvent) {
    ...
    if (
      ...
      (!this.isMobile && [1, 2].includes((e as MouseEvent).buttons)) ||
      (this.isMobile &&
  ...
  public handleMouseUp(e: MouseEvent | TouchEvent) {
    if (this.lock) return;
    if (this.isMobile) {
   ...
   * 调试断言工具
   */
  private assert(condition: boolean, message: any) {
    if (!condition) {
