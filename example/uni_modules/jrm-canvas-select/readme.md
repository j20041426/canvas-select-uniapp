...
本项目是基于web端canvas-select库改造而来的的uni-app组件，用于图片标注，可适配H5、APP和微信小程序（只支持触摸环境）。

# 安装

在uni-app的插件市场进行安装，本项目支持uni_modules安装。
...
拿到实例后，即可调用原canvas-select库的方法，由于是在触摸环境下使用，所以`删除了`放大镜相关的功能。
同时新增了一个导出图片的方法：`exportImage`。

```javascript
/**
 * 导出图片
 * @param {Object} options - 导出选项
 * @param {string} options.fileType - 图片类型，默认jpg
 * @param {number} options.quality - 图片质量，默认0.9
 * @returns {Promise<string>} H5环境下返回图片base64编码，APP和小程序环境下返回图片tempFilePath路径
 */
instance.exportImage({ fileType: "jpg", quality: 0.9 });
```

以下是canvas-select库的文档：
...
| `loading` |    `src` (图片链接)     |                        开始加载背景图片时触发                         |
| `load`    |    `src` (图片链接)     |                        背景图片加载完成时触发                         |
